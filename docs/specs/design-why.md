...
> **작성일:** 2026-06-19 | **최종 갱신:** 2026-06-23  
> **배경:** Audiveris OMR 심층 진단 결과, 찬송가 315장 소프라노 검출 정확도 65% (52음 중 34음 검출, 4개 마디 전체 누락). 규칙 기반 OMR의 한계 확인 → 딥러닝 기반 재개발 결정.  
> **연계:** [ROADMAP.md](../ROADMAP.md) | [OMR 학습 파이프라인 설계](aiscore-system-design.md) | [CLAUDE.md](../../CLAUDE.md)  
> **역할:** 설계 원칙·아키텍처·평가 기준·기술 선택 근거 (Why). 구체적인 소스 구조·개발 절차는 [학습 파이프라인 설계](aiscore-system-design.md) 참조.
...
### 4.0 보유 Ground Truth (2026-06-23 확정)

| 자산 | 경로 | 수량 | 비고 |
|------|------|------|------|
| 악보 이미지 | `score_images/png/hymn{NNN}_Normal.png` | 645개 | 800×1248px |
| 분리악보 XML | `score_images/xml/분리/새찬송가_{NNN}*.xml` | 644개 | NWC→music21 변환 |

**페어링:** `hymn{NNN}_Normal.png` ↔ `새찬송가_{NNN}*.xml`  
마디 단위 크롭 시 644쌍 × ~12마디 ≈ **7,700+ 학습 샘플** 확보 가능.

### 4.1 데이터 소스

| 소스 | 종류 | 규모 | 획득 방법 |
|------|------|------|---------|
| **✅ 보유 NWC→XML** (0순위) | 찬송가 이미지+XML 644쌍 | 확보 완료 | `score_images/` |
